...
const InputForm = ({
  ...
  const handleFileUpload = async () => {
    try {
      ...
      if (result && result.length > 0) {
        ...
        // User cancelled or did not select a file; do nothing and do not log
        return;
      }
    } catch (error) {
      // Handle user cancel (by code or message) -- do not log or show any error
      if (
  ...
  // handleCameraCapture removed; not needed anymore
        ...
        <View style={styles.cameraContainer}>
          ...
          <View style={styles.cameraBox}>
            ...
            <CustomText style={styles.cameraPrompt}>
              {capturedImages.length > 0 ? `${capturedImages.length} photo(s) captured` : 'Use the buttons below to add photos'}
            </CustomText>
